...
<html lang="en">
  <head>
    ...
    <!-- Title-->
    <title>Review Order || <?=$co_name;?></title>
  ...
  <body>
    ...
    <?php
		$query=mysqli_query($con,"select * from health_package_detail where id='".$_GET['plan_id']."'");
		$res=mysqli_fetch_array($query);
		$plan_id=$res['id'];
		$package_id=$res['health_package_id'];
		...
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					...
					<p style="background:teal;padding:13px 20px;border-radius:15px;color:white;font-weight:600"><i class="fa fa-circle" aria-hidden="true" style="font-size: 18px;vertical-align: text-bottom;"></i>&nbsp;Mudra Care Membership <span style='float:right;'>₹<?=number_format($price_without_tax,2);?></span></p>
					<p style="margin-top:23px;letter-spacing:1px;;">Payment Details</p>
					<p style='color:black;margin-bottom:10px;'>Mudra Care Membership - <?=$duration;?> months<span style='float:right;'>₹<?=number_format($price_without_tax,2);?></span></p>
		...
		function continue_to_payment_mode()
			{
				...
				window.location.href="payment-mode.php?to_pay="+to_pay+"&plan_id="+plan_id;
			}	
	</script>
  </body>
</html>
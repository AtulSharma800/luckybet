			...
			if(!empty($_FILES['main_image']['name']))
				{
					...
					$query2=mysqli_query($con,"update booked_test set report_file='$main_image' where id='$id'");
					$res2=mysqli_affected_rows($con);
				}
			
			//insert package details
			$query=mysqli_query($con,"update booked_test set member_id='$user_id',lab_id='$lab_id',test_id='$test_id',test_name='$test_name',amount='$amount',payment_mode='$payment_mode',payment_status='$payment_status',status='$status',modified_at='$modified_at' where id='$id'");
...
<body>
			...
			<div class="content">
				...
				<div class="row">
					<div class="col-xl-12">
						...
						<?php
							$id=$_GET['id'];
							$query=mysqli_query($con,"select * from booked_test where id='$id'");
							$res=mysqli_fetch_array($query);
						?>	
						<form action="booked-test-edit.php?id=<?=$id;?>" method="post" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
									...
									<fieldset>
										<div class="row">	
											...
											<div class="col-md-3">
												<div class="form-group">
													<label>Amount (₹)</label>
													<input type="text" name="amount" value="<?=$res['amount'];?>" class="form-control">
												</div>	
											</div>
											<div class="col-md-3">
												<div class="form-group">
											...
											<div class="col-md-3">
												<div class="form-group">
													...
													<select type="text" name="payment_status" class="form-control">
														...
														<option value='Not Paid' <?php if($res['payment_status']=='Not Paid'){echo 'selected';} ?>>Not Paid</option>
													</select>
												</div>	
											</div>
											<div class="col-md-3">
												<div class="form-group">
													...
													<select name="status" id="status" class="form-control">
														...
														<option value="Confirmed" <?php if($res['status']=='Confirmed'){echo 'selected';} ?>>Confirmed</option>
														...
														<p style="font-size:24px;text-align:center;">
															<?php
																if(!empty($res['image']))
																	{
															?>
																		<img src="<?php echo 'upload/booked_test/'.$res['image'];?>" style="width:100%;max-height:200px;object-fit:contain;display:block">
															<?php
																	}
																else
																	{															
	...
	<?php
		if($update_status==1)
			{
				echo "<script>$.Toast('', 'Booked Test updated successfully.', 'success', {
										has_icon:false,
										has_close_btn:true,
										stack: true,
										fullscreen:true,

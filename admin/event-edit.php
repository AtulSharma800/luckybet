			...
			if(!empty($_FILES['main_image']['name']))
				{
					...
					move_uploaded_file($_FILES['main_image']['tmp_name'],"upload/city/$main_image");
					$query=mysqli_query($con,"update events set image='$main_image' where id='$id'");
				}
					...
					if(!empty($_FILES['gallery']['name'][$i]))
						{
							...
							move_uploaded_file($_FILES['gallery']['tmp_name'][$i],"upload/event_gallery/$main_image");
							mysqli_query($con,"insert into event_gallery(event_id,image) values('$event_id','$main_image')");
							$i++;
						}
				}
			$query=mysqli_query($con,"update events set title='$title',description='$description',location='$location',date_added='$date',time_added='$time' where id='$id'");
...
<head>
	...
	<title>Edit Event :: <?=$co_name;?></title>
			...
			<div class="page-header page-header-light">
				<div class="page-header-content header-elements-md-inline">
					<div class="page-title d-flex">
						<h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Home</span> - Edit Event</h4>
			...
			<div class="content">
				...
				<div class="row">
					<div class="col-xl-12">
						...
						<div class="card">
							<div class="card-header header-elements-sm-inline">
								...
								<div class="header-elements">
									<a onclick="events.update()"><span class="badge bg-info badge-pill" style="cursor:pointer;font-size:14px;">UPDATE</span></a><a href="event.php"><span class="badge bg-success badge-pill" style="font-size:14px;">EVENT LIST</span></a>
								</div>
							</div>
							<div class="card-body">
						...
						<?php
							$id=$_GET['id'];
							$query=mysqli_query($con,"select * from events where id='$id'");
							$res=mysqli_fetch_array($query);
						?>	
						<form action="event-add.php?id=<?=$id;?>" method="post" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
							<div class="card card-body border-top-0 rounded-0 rounded-bottom tab-content mb-0">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												...
												<div class="main_image img-thumbnail" onclick="event2.select_image()">
													<p style="font-size:24px;text-align:center;">
														<?php
															if(!empty($res['image']))
																{
														?>
																	<img src="<?php echo 'upload/event/'.$res['image'];?>" style="width:100%;display:block">
														<?php
																}
															else
																{															
										...
										<div class="row">	
											...
											<div class="col-md-12">
												<div class="form-group">
													...
													<input type="text" name="location" class="form-control" value="<?=$res['location'];?>">
												</div>	
											</div>
											<div class="col-md-6">
												<div class="form-group">
													<label>Event Date</label>
													<input type="date" name="event_date" class="form-control" value="<?=$res['date_added'];?>">
												</div>	
											</div>
											<div class="col-md-6">
												<div class="form-group">
													<label>Event Time</label>
													<input type="time" name="event_time" class="form-control" value="<?=$res['time_added'];?>">
												</div>	
											</div>
								...
								<div class="tab-pane" id="tab-dark-2">
									
									<div class="col-md-12">
										<div class="gallery2 row" >
											<?php
												$query2=mysqli_query($con,"select * from event_gallery where event_id='$id'");
												while($res2=mysqli_fetch_array($query2))
													{
														$image=$res2['image'];
														$id2=$res2['id'];
														echo "<div class='col-md-3' id='gallery_photo$id2'><div class='row'><img class='img-thumbnail img-responsive' src='upload/event_gallery/$image'><span class='btn btn-danger' style='position: absolute;right: 28px;padding: 2px 10px 0px 10px;margin-top: 0px;margin-top: 22px;' onclick='event2.delete_gallery($id2)'><i class='fa fa-trash-o'></i></span></div></div>";
													}
											?>
										</div>
